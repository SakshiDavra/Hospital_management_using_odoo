...
class PurchaseOrder(models.Model):
    ...
    def action_upload_invoice(self, file_name=False, file_type=False, file_data=False, from_model="purchase.order", picking_id=False, **kwargs):
        ...
        if from_model == "purchase.order":
            if self.state in ("draft", "sent"):
                return self._sync_draft_po_from_invoice(ocr_result)
            return self._update_po_qty_from_invoice(ocr_result)
    ...
    def _create_vendor_bill_after_receipt(self, ocr_result):
        ...
        if validation:
            return validation
        return self._create_vendor_bill_from_saved_ocr()
    ...
    def _sync_draft_po_from_invoice(self, ocr_result):
        ...
        for item in ocr_result.get("lines", []):
            ...
            clean_invoice_name = re.sub(r"^\d+[\s\.\-\)]+", "", raw_name).strip()
            best_line = self._find_best_po_line(clean_invoice_name, used_po_lines)
            ...
            if best_line:
                ...
                if float_compare(best_line.product_qty, invoice_qty, precision_rounding=best_line.product_uom_id.rounding) != 0:
                    mismatched_lines.append(Command.create({
                        "po_line_id": best_line.id,
                        "product_id": best_line.product_id.id,
                        "po_qty": best_line.product_qty,
                        "invoice_qty": invoice_qty,
                        "price_unit": float(item.get("price") or best_line.price_unit),
                        ...
                        if not tax:
                            try:
                                tax = self.env["account.tax"].create({
                                    "name": f"Purchase GST {tax_rate:g}%",
                                    "amount_type": "percent",
                                    "amount": tax_rate,
                                    "type_tax_use": "purchase",
                                    "company_id": self.company_id.id,
        ...
        if mismatched_lines:
            wizard = self.env["purchase.qty.wizard"].create({
                "purchase_id": self.id,
                "line_ids": mismatched_lines,
            })
            view = self.env.ref("vendor_bill_ocr_automation.view_purchase_qty_wizard_form")
            return {
                "type": "ir.actions.act_window",
                "name": _("Quantity Mismatch Confirmation"),
                "res_model": "purchase.qty.wizard",
                "res_id": wizard.id,
                "view_mode": "form",
                "views": [(view.id, "form")],
                "target": "new",
    ...
    def _find_best_po_line(self, invoice_name, used_po_lines):
        ...
        clean_invoice_name = re.sub(r"^\d+[\s\.\-\)]+", "", (invoice_name or "").lower().strip()).strip()

        for po_line in self.order_line:
            if po_line.id in used_po_lines:
                continue

            po_product_name = (po_line.product_id.display_name or "").lower().strip()
            po_product_name = re.sub(r"^\d+[\s\.\-\)]+", "", po_product_name).strip()
    ...
    def _check_bill_validation(self, ocr_result):
        ...
        done_incoming_moves = self.picking_ids.filtered(
            lambda p: p.picking_type_id.code == "incoming" and p.state == "done"
        ).move_ids

        for item in ocr_result.get("lines", []):
            invoice_qty = float(item.get("qty") or 0.0)
            po_line = self._find_best_po_line((item.get("name") or "").lower().strip(), used_po_lines)
            ...
            if (float_compare(po_qty, receipt_qty, precision_rounding=po_line.product_uom_id.rounding) != 0 or 
                float_compare(receipt_qty, invoice_qty, precision_rounding=po_line.product_uom_id.rounding) != 0):
                mismatch_lines.append(Command.create({
        ...
        wizard = self.env["purchase.bill.validation.wizard"].create({
            "purchase_id": self.id,
            "line_ids": mismatch_lines,
        })
        view = self.env.ref("vendor_bill_ocr_automation.view_purchase_bill_validation_wizard_form")
        return {
            "type": "ir.actions.act_window",
            "name": _("Vendor Bill Validation"),
            "res_model": "purchase.bill.validation.wizard",
            "res_id": wizard.id,
            "view_mode": "form",
            "views": [(view.id, "form")],
            "target": "new",
    ...
    def _create_po_line_from_invoice(self, item):
        self.ensure_one()
        ocr_product_name = re.sub(r"^\d+\s*[\.\-\)]*\s*", "", re.sub(r"\s+", " ", (item.get("name") or "").replace("\n", " ").strip()))
    ...
    def _update_po_qty_from_invoice(self, ocr_result):
        ...
        for item in ocr_result.get("lines", []):
            ...
            best_line = self._find_best_po_line((item.get("name") or "").lower().strip(), used_po_lines)
            if not best_line:
                mismatched_lines.append(Command.create({
                    "invoice_name": item.get("name"),
                    "invoice_qty": invoice_qty,
    ...
    def _validate_invoice_against_po(self, ocr_result):
        ...
        for item in ocr_result.get("lines", []):
            invoice_name = (item.get("name") or "").lower().strip()
            best_line = self._find_best_po_line(invoice_name, used_po_lines)
            if best_line:
                matched += 1
                used_po_lines.add(best_line.id)
            else:
    ...
    def _prepare_ocr_invoice_line(self, item, used_po_lines, pdf_tax_rate=0.0):
        self.ensure_one()
        raw_product_name = re.sub(r"\s+", " ", item.get("name", "Default Product").replace("\n", " ").strip())
        ...
        if match := re.search(r'\(\s*(\d+)\s*(?:pieces|pcs|pices|nos)?\s*x\s*[\$\s]*([\d\.]+)\s*\)', raw_product_name, re.IGNORECASE):
            ...
            product_name = clean_name or f"Laundry Service {bracket_string}"
        else:
            product_name = raw_product_name

        product_name = re.sub(r'^\d+[\s\.\-\)]+', '', product_name).strip() or "Laundry Service"
        po_line = self._find_best_po_line(product_name.lower().strip(), used_po_lines)
        ...
        if po_line:
            ...
            product = self.env["product.product"].search([("name", "=ilike", product_name)], limit=1) or self.env["product.product"].create({
                "name": product_name, "purchase_ok": True, "sale_ok": False, "type": "consu"
            })
        ...
        if pdf_tax_rate > 0:
            tax = self.env["account.tax"].search([("amount", "=", pdf_tax_rate), ("type_tax_use", "=", "purchase"), ("company_id", "=", self.company_id.id)], limit=1)
            if not tax:
                try:
                    tax = self.env["account.tax"].create({
                        "name": f"Purchase GST {pdf_tax_rate:g}%", "amount_type": "percent", "amount": pdf_tax_rate,
                        "type_tax_use": "purchase", "company_id": self.company_id.id, "sequence": 10
                    })
                except Exception:
                    pass
            if tax:
