...
class ProductPricelist(models.Model):
    ...
    def _is_yearly_occurrence(self, today):
        ...
        occ = list(rule.between(datetime.combine(start_date, time.min), datetime.combine(today, time.max), inc=True))
        if not occ:
            return False
        last = occ[-1].date()
        return last <= today <= last + (self.schedule_end_date - self.schedule_start_date)
    ...
    def write(self, vals):
        ...
        for record in self:
            if record.recurrency:
                if rv or 'recurrency' in vals or 'rrule_type_ui' in vals:
    ...
    def _onchange_end_type(self):
        for r in self:
            if r.end_type == "count":
                ...
                r.count = 0
                r.until = r.until or fields.Date.today()
    ...
    def _onchange_enable_schedule(self):
        for r in self:
            if not r.enable_schedule:
                r.schedule_start_date = False
                r.schedule_end_date = False

    @api.onchange("enable_time_slot")
    def _onchange_enable_time_slot(self):
        for r in self:
            if not r.enable_time_slot:
                r.time_slot_start = False
                r.time_slot_end = False
    ...
    def _check_pricelist_conflict(self):
        for r in self:
            if not (r.enable_schedule and r.enable_time_slot and r.schedule_start_date and r.schedule_end_date):
                continue
            others = self.search([
                ("id", "!=", r.id), ("name", "=", r.name),
                ("enable_schedule", "=", True), ("enable_time_slot", "=", True),
                ("schedule_start_date", "<=", r.schedule_end_date),
                ("schedule_end_date", ">=", r.schedule_start_date),
            ])
            for other in others:
                if r._conflicts_with(other):
                    raise ValidationError(_("A duplicate pricelist schedule already exists for '%s'.") % other.display_name)

    def _conflicts_with(self, other):
        self.ensure_one()
        overlap = self.time_slot_start < other.time_slot_end and self.time_slot_end > other.time_slot_start
        if not overlap:
            return False
